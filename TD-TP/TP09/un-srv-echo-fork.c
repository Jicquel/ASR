...
void* c_fonction(void* arg){
  ...
  int nbProduitEnleve;
  while(1){
    nbBytes=recv(nIdS,nbProduitEnleve,4,0);
    pthread_lock_mutex(&vitrine);
...
void* p_fonction(void* arg){
  ...
  while(1){
    nbBytes=recv(nIdS,nbProduitRajoute,4,0);
    pthread_lock_mutex(&vitrine);
...
void* producteur(void* arg){
  ...
  strncpy(socket_Prod.sun_path,"/tmp/serv",sizeof(struct sockaddr_un)-2);
